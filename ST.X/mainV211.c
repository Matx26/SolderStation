 ...
 *            una pausa de 5.5us.
 * main_V210: Esta version implementara un control PID sobre un horno (oven),
 *            La frec de muestreo sera de 1Khz
 * main_V211: Esta version implementara hara una mix entre la nueva V110 y la V210
 *            
 * 
 ...
 * contraste.
 *  Created on 29 de junio de 2021, 11:10
 */
...
#include "lcd16x4.h"       //se debe incluir esta libreria despues de definir FCY e incluir
...
int ADCValue=0;
float PWMControl=0, a=0.1243, b=0.000062, c=62.15, SetPoint=350, LimMaxTemp=16382, LimMinTemp=0;
float rT=0, eT=0, iT=0, dT=0, yT=0, uT=0, iT0=0, eT0=0, iT_1=0, eT_1=0;
/*ADCValue      -> almacena la valor leido por el ADC
 *PWMControl    -> parametro que define la salida
 *a, b, c, d    -> constante del sitema
 *SetPoint      -> temperatura de trabajo
 * LimMaxTemp   -> limite maximo para el anti-windup, es 1023 porque la resolucion del PWM es de 10bits por ende 1023 es el valor maximo 
...
void Config_Port_Mcu(void)
{
    ...
    TRISBbits.TRISB15 = 0;     //se usa para apagar la senal PWM a traves una compuerta AND
...
void Config_OutputCompare1(void)
{
    ...
    OC1R = 0;      //inicializamos el registro de duty cycle que se usa al inicial la senal PWM
    OC1RS = 0;      //inicializamos el segundo registro duty cycled que se usa una ves iniciado el PWM
    ...
    PR2 = 15999;     //especificamos el valor de PR2 (del TIMER2) para obtener un periodo de la señal pwm de 64us
    ...
    TMR2 = 0x0000;      // borramos el contenido del timer3
...
{   
    ...
    PR3 = 13600; //15840;        //PR3 = 13600 -> Periodo = 850us. se usa 150us porque el calculo del PID demora 108us 
    //T2CONbits.TON = 1;  //habilitamos el timer2 y por ende al PWM1
}
...
/***************************Config_AD******************************************/
/* Funcion: configura el modulo AD de modo que realice la interrupcion por cada*/
/*          conversion, se usa Timer3 como trigger.   */
/* Argumentos: no tiene                                                       */
/* Retorna: nada                                                              */
/******************************************************************************/
void Config_AD(void)
...
void ConfigInterrupAD(void)
{
    ...
    IPC3bits.AD1IP = 0b011;    //seleccionamos la prioridad como 3
...
void __attribute__((interrupt, auto_psv)) _ADC1Interrupt(void)
{
    ...
    TMR3 = 0;       //reseteamos la cuenta del timer 3
    
    LATBbits.LATB15 = 0;    //se pone RB15 en 0, para DESHABILITAR LA SALIDA pwm
    LATBbits.LATB13 = 1;    //se pone RB13 en 1, para medir el tiempo que lleva hacer el calculo
    ...
    ADCValue = ADC1BUF0;      //tranfiero el valor del ADC 
    yT = ADCValue / 2;      //512 grados cent  -> 3,3V -> 1024(10 bits) => x(en grados centigrado)=ADCvalue*512/1024=ADCValue/2
    rT = SetPoint;          //rT almacena el punto de trabajo
    eT = rT - yT;       //calculo el error entre el set point y la realimentacion
    iT = b * eT + iT0;  //calcula el termino integral
    dT = c * (eT - eT0);    //calcula el termino derivativo
    uT = a*eT + iT + dT;    //calculo la salida del controlador PID
    
    if(uT > LimMaxTemp)     //control anti windaup
    {
        uT = LimMaxTemp;
    }
    else
    ...
    OC1RS = (uT*5);     //escribimos en el registro que define el ciclo de trabajo de la señal PWM
    iT0 = iT;       //guardamos loss valores que seran la muestra anterior
    eT0 = eT;       //
    
    LATBbits.LATB13 = 0;    //se RB13 en 0 para medir el tiempo de calculo
    LATBbits.LATB15 = 1;    //se pone RB15 en 0, para DESHABILITAR LA SALIDA pwm
}
...
int main(void) 
{
    ...
    {   // 
        ...
        if(Sensor_Mag == 1)
        {
            if(Count_Sleep >= (10 * Sleep_Delay))
            {
                if(menu != 1)
                {
                    SetPoint = Sleep_Temp;   //SetPoint es la variable que usa el PID, osea se le pasa la nueva temperatura de trabajo    
                    Menu1();    // llamo a la funcion que escribe en el LCD el menu1 (menu de Sleep)
                    menu = 1;   //indico que estamos en el menu 1
                }
                else
                {
                    if(Count_Sleep >= (600 * Hib_Delay))
                    {
                        ...
                        Menu2();    // llamo a la funcion que escribe en el LCD el menu2 (menu de hibernacion)
                        menu = 2;   //indico que estamos en el menu 2 
                    }
                }
            }
            
            Count_Sleep = Count_Sleep + 1;      //incremeto el contador de para saber cuando debe entrar en modo invernacion 
            __delay_ms(100);     //aca esperamos un tiempo de 100milisegundos
            
        }
        else
        {
            Count_Sleep = 0;    //antes de salir del modo Sleep o Hibernacion se resetea el contador
            if(menu == 1 || menu == 2)   //si el menu es Ibernacion o Sleep volvemos a la pantalla de normal (menu0)
            {
                SetPoint = Set_Temp;       //SetPoint es la variable que usa el PID, ose se le pasa la nueva temperatura de trabajo 
                Menu0();    // llamo a la funcion que escribe en el LCD el menu0
                menu = 0;   //indico que estamos en el menu 0
            }
